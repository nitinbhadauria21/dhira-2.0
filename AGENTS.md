...
- **What this is:** a Next.js 15 / React 19 / TypeScript app with a real backend layer. It runs in two modes automatically:
  ...
  - **Live mode:** set a real `ANTHROPIC_API_KEY` (starts with `sk-`) to use the six Claude agents; set the Supabase keys to save to Postgres and use real Auth instead of the local file / dev auth. Put real keys in `.env.local` (git-ignored) — the committed `.env` holds placeholders only. Check current mode via `GET /api/status`.
