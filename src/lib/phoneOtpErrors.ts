...
export function formatPhoneOtpSendError(raw: string): string {
  ...
  if (
    ...
    return (
      ...
      'In Supabase → Authentication → Providers → Phone, either choose **Twilio Verify** and paste that VA Service SID in the Verify field, '+ 'or choose **Twilio** and set a real Twilio phone number in E.164 (e.g. +1…) — not the VA ID. See docs/SUPABASE_PHONE_OTP.md § “Invalid From / VA…”.'
    );
  }
  ...
  if (
    ...
    return (
      'This phone number cannot receive SMS yet because your Twilio account is on a free trial. ' + 'In Twilio Console → Phone Numbers → Verified caller IDs, add this exact number (+91… in E.164), confirm the code Twilio texts you, then tap Send verification code again. '+ 'For real users in production, upgrade the Twilio account (or use a paid messaging setup). See docs/SUPABASE_PHONE_OTP.md § Twilio trial (21608).'
    );
  }
