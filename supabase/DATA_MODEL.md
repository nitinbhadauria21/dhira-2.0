...
graph TD
  ...
  P --> PCols["alias, avatar, language, timezone, email, phone_e164, preferred_channel, email_opt_in, whatsapp_opt_in, consent_checkin, consent_memory, checkin_frequency, checkin_window, last_proactive_at, last_weekly_at"]
  ...
  P --> NOT["notifications: channel, type, content, status, provider_message_id, template_key, subject, scheduled_for, sent_at, created_at"]
```
...
```mermaid
graph LR
  Trig["Emergent schedule OR in-app trigger"] --> Due["GET /api/notifications/due"]
  Due --> Consent["consent + channel check on profile"]
  ...
  Row --> Emg["POST to Emergent webhook email or WhatsApp"]
  Emg --> Cb["status callback updates row sent delivered failed"]
```

## Tables

- profiles — one row per user (id = `auth.users.id`). Alias/avatar/language,
  contact (email, phone_e164), channel preference + opt-ins, timezone, the
  check-in contract (consent + frequency + window), and `last_proactive_at` /
  `last_weekly_at` for Emergent due-lists.
...
- notifications — each outbound email/WhatsApp message, template_key/subject, and delivery status.
...
Demo Day guide (plain English, attach in Emergent):
...
`supabase/migrations/20260720_notification_orchestration.sql`
